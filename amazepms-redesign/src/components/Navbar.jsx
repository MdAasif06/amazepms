...
export default function Navbar() {
  ...
  return (
    <motion.header
      ...
      className="fixed inset-x-0 top-0 z-50 min-w-7xl flex justify-center px-4 pt-3 sm:pt-4"
    >
      <nav
        className={cn(
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {links.map((link) => (
            <a
              ...
              className="rounded-full px-3.5 py-2 text-sm text-muted-foreground transition-colors duration-200 hover:bg-white/5 hover:text-foreground"
            >
              {link.label}
            </a>
          ))}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute inset-x-4 top-19 rounded-2xl glass-strong p-4 shadow-card lg:hidden"
          >
